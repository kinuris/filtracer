...
<div id="unverifyModal" class="fixed z-50 inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center hidden">
    <div class="bg-white p-6 rounded-lg shadow-lg w-1/3 min-w-[500px] max-h-[max(85%,600px)] overflow-auto">
        ...
        <!-- Edit Mode -->
        <form id="unverifyEditMode" class="hidden" action="/admin/useraccount/update/{{ $user->id }}" method="POST">
            @csrf
            <div class="grid grid-cols-2 gap-3">
                @if ($user->role != 'Admin')
                <div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const editButton = document.getElementById('unverifyEditButton');
        const viewMode = document.getElementById('unverifyViewMode');
        const editMode = document.getElementById('unverifyEditMode');
        const cancelEdit = document.getElementById('cancelUnverifyEdit');
